...
int ModuleFonts::Load(const char* texture_path, const char* characters, uint rows)
{
	...
	uint height_texture = 0;
	uint width_texture = 0;
	App->textures->GetSize(tex, width_texture, height_texture);
	...
	fonts[id].char_w = width_texture / fonts[id].row_chars;
		fonts[id].char_h = height_texture / rows;
...
void ModuleFonts::BlitText(int x, int y, int font_id, const char* text) const
{
	...
	for(uint i = 0; i < len; ++i)
	{
		...
		App->render->Blit(font->graphic, x*(i+font->char_w), y, &rect, 0.25f, false);
	}
}
